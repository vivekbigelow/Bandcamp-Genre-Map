...
if (process.env.NEXT_PUBLIC_GOOGLE_MAP_PUBLIC_KEY) {
  key = process.env.NEXT_PUBLIC_GOOGLE_MAP_PUBLIC_KEY;
}
        ...
        >
          {data.genreByName.albums.map((g_album) => (
            <Marker
              ...
              lat={g_album.location.lat + Math.random() * 0.008}
              lng={g_album.location.lng + Math.random() * 0.008}
            />
          ))}
        </GoogleMapReact>
      </div>
